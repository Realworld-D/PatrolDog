...
class Paddle_inference:
    def __init__(self, model_folder_dir, use_model_img_size=True, 
        ...
        self.precision_mode = precision_mode       # TensorRT的precision_mode
    ...
    def get_model_path(self):
        ...
        if os.path.isdir(self.model_folder_dir):
            ...
            for file in os.listdir(self.model_folder_dir):
                file_type = os.path.splitext(file)[1]    # 分离文件名与扩展名
    ...
    def init(self,camera_shape=(640,480)):
        img = np.zeros(shape=(int(camera_shape[1]), int(camera_shape[0]),3),dtype="float32")
        # 读取模型文件
        self.get_model_path()
        ...
        self.mean, self.std, self.im_scale, self.scale_factor, self.infer_im_shape = self.img_config_init(img, target_size, mean, std)
        # 滤波模式：初始化检测滤波器
        if self.filter_mode:
    ...
    def infer(self, img):
        ...
        # result = self.predict(self.predictor, [data, self.scale_factor])
        return result

    # ————————————————读取yml参数信息———————————————— #
    def infer_yml_config(self):
        infer_cfg = open(self.infer_cfg_file)
        data = infer_cfg.read()
        yaml_reader = yaml.load(data)
        self.label_list = yaml_reader['label_list'] # 字典索引
        ...
        mean = [0.485, 0.456, 0.406] #yaml_reader['Preprocess'][1]['mean'] #
        std =  [0.229, 0.224, 0.225] #yaml_reader['Preprocess'][1]['std'] # 
    ...
    def predict_config(self):
        ...
        if self.use_gpu == True:
            ...
            print("Precision mode: {}".format(self.precision_mode)) # 0
            print("----------------------------------------------") 
            if self.use_tensorrt == True:
                if self.precision_mode == PrecisionType.Int8:
                    use_calib_mode = True
                    use_static = True
                if self.precision_mode == PrecisionType.Half:
                    use_calib_mode = False
                    use_static = True
                else:
                    use_calib_mode = False
                    use_static = False
                config.enable_tensorrt_engine(workspace_size=1 << 30, precision_mode=self.precision_mode,
